...
function Vendor() {
    ...
    useEffect(() => {
        ...
        console.log("Vendor Data from sessionStorage:", ven);
        ...
        console.log("JWT Token:", ven.jwt);

        const config = {
            headers: {
                Authorization: `Bearer ${ven.jwt}`
            }
        };

        axios.get(`${IP_ADDRS}/vendors/${ven.id}`, config)
            .then((res) => {
            ...
            {loggedIn ? (
                <>
                    <div className="jumbotron" style={{ marginLeft: 20 }}>
                        ...
                        <h5 style={{ marginLeft: 30 }}>{vendor.email}</h5>
                        <br />
                    </div>
                    <hr className="my-4" />
